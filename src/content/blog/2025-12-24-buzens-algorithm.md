...
Fifty jobs circulating among ten servers can occupy over 12 billion distinct configurations. Yet one algorithm computes the exact probability of every such state with only a few hundred operations. This speedup - from billions of states to hundreds of multiplications - comes from the product-form structure of Jackson networks and a convolution trick discovered by Jeffrey Buzen in 1973.

This post covers closed queueing networks, the normalization constant problem, and a detailed derivation of Buzen's algorithm with implementation.

## The Setup

Networks of queues appear everywhere - packets hopping between routers, jobs cycling through CPU and disk, requests flowing through microservices. By the 1960s, queueing theorists discovered something surprising: under certain conditions, a network of queues reaches a steady state where each queue behaves as if it were independent.

**Open networks** have external arrivals and departures. Jackson's 1963 theorem showed that for $M$ queues with Poisson arrivals and exponential service, the steady-state probability factors into a product:

$$P(k_1, \ldots, k_M) = \prod_{i=1}^M (1-\rho_i)\rho_i^{k_i}$$

where $\rho_i = \lambda_i/\mu_i$ is the utilization at node $i$. Each queue behaves like an isolated M/M/1 queue.

**Closed networks** have a fixed population - $N$ jobs circulate forever, never entering or leaving. Think of $N$ processes alternating between CPU and I/O, or a fixed pool of database connections. Gordon and Newell extended Jackson's result in 1967: closed networks also have product-form solutions, but with a catch.

For $N$ customers distributed among $M$ queues, the probability of state $(k_1, \ldots, k_M)$ where $\sum_i k_i = N$ is:

$$\pi(k_1, \ldots, k_M) = \frac{1}{G(N)} \prod_{i=1}^M X_i^{k_i}$$

where $X_i = e_i/\mu_i$ combines the visit ratio $e_i$ (relative arrival rate to node $i$) and service rate $\mu_i$. The visit ratios come from solving the traffic equations $e_j = \sum_i e_i p_{ij}$ for the routing matrix.

The denominator $G(N)$ is the **normalization constant** - chosen so probabilities sum to 1:

$$G(N) = \sum_{\mathbf{k}: \sum k_i = N} \prod_{i=1}^M X_i^{k_i}$$

This sum runs over all ways to distribute $N$ customers among $M$ queues. For $N=50$ and $M=10$, that's $\binom{59}{9} \approx 1.25 \times 10^{10}$ terms.

Computing $G(N)$ is the bottleneck.

## The Naive Approach

Direct enumeration iterates over every state $(k_1, \ldots, k_M)$ with $\sum k_i = N$, computes $\prod X_i^{k_i}$, and accumulates the sum. The number of states grows as:
...
A network of 10 queues and 50 customers requires iterating over tens of billions of states. Even modern hardware chokes on this.

The structure of the sum suggests redundancy. Each term $\prod_i X_i^{k_i}$ shares factors with many others. The sum over all distributions is essentially an $M$-fold convolution - a hint that dynamic programming might help.

## The Insight

Buzen recognized that $G(N)$ can be computed recursively by adding one queue at a time.

Define $g(n, m)$ as the normalization constant for $n$ customers distributed among the first $m$ queues:

$$g(n, m) = \sum_{\mathbf{k}: \sum_{i=1}^m k_i = n} \prod_{i=1}^m X_i^{k_i}$$

By definition, $g(N, M) = G(N)$.

Now partition the states based on queue $m$'s occupancy. Either queue $m$ has zero customers, or it has at least one:

- **Zero at queue $m$**: Queue $m$ contributes $X_m^0 = 1$. The remaining $n$ customers are distributed among queues $1$ through $m-1$. This contributes $g(n, m-1)$.

- **At least one at queue $m$**: Factor out $X_m$ for one customer at queue $m$. The remaining $n-1$ customers (including any additional ones at queue $m$) distribute among all $m$ queues. This contributes $X_m \cdot g(n-1, m)$.
...
Base cases:
- $g(0, m) = 1$ for all $m$ (one way to place zero customers - all queues empty)
- $g(n, 0) = 0$ for $n > 0$ (no queues means no way to place customers)

This recurrence computes $g(N, M)$ using only $O(NM)$ operations instead of $O\left(\binom{N+M-1}{M-1}\right)$.
...
The inner loop implements the recurrence in-place. When computing `g[n]`:
- `g[n]` on the right side holds $g(n, m-1)$ from the previous iteration
- `g[n-1]` holds $g(n-1, m)$ - already updated in the current iteration

Both terms of $g(n,m) = g(n,m-1) + X_m \cdot g(n-1,m)$ come from the correct places.
...
The probability that queue $i$ has at least $k$ customers:

$$P(n_i \geq k) = X_i^k \cdot \frac{G(N-k)}{G(N)}$$

Intuition: fix $k$ customers at queue $i$ (contributing $X_i^k$), distribute the remaining $N-k$ arbitrarily (summing to $G(N-k)$), normalize by $G(N)$.

```python
def prob_at_least(g: list[float], X: list[float], i: int, k: int) -> float:
    """P(queue i has >= k customers)"""
    N = len(g) - 1
    if k > N:
        return 0.0
...
The expected number of customers at queue $i$:
...
| Approach | Time | Space |
|----------|------|-------|
| Naive enumeration | $O\left(\binom{N+M-1}{M-1}\right)$ | $O(1)$ |
...
1. **Closed queueing networks** with exponential service have product-form stationary distributions (Gordon-Newell theorem), but computing the normalization constant $G(N)$ requires summing over exponentially many states.

2. **Buzen's recurrence** $g(n,m) = g(n,m-1) + X_m \cdot g(n-1,m)$ computes $G(N)$ in $O(NM)$ time by adding one queue at a time - a reduction from billions of operations to hundreds.

3. **Dynamic programming is optimal** for this structure. FFT-based polynomial multiplication would actually be slower due to the log factor.

4. **Performance metrics** (queue lengths, throughput, utilization) derive directly from the $G$ array without additional state enumeration.

5. **The algorithm is remarkably simple** - a double loop with a single multiply-add per iteration. The insight is recognizing that a massive combinatorial sum decomposes into sequential convolutions.
